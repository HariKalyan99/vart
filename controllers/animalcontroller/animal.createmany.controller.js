    ...
    upload(request, response, async (err) => {
        ...
        try {
            ...
            fs.createReadStream(filePath)
                ...
                .on('end', async () => {
                    ...
                    for (const row of results) {
                        ...
                        if (!animalname || !email || !phoneNumber) {
                            validationErrors.push(`Missing mandatory fields in row: ${JSON.stringify(row)}`);
                            continue;
                        }

                        if (!validator.isEmail(email)) {
                            validationErrors.push(`Invalid email in row: ${JSON.stringify(row)}`);
                            continue;
                        }

                        if (!validator.isMobilePhone(phoneNumber) || phoneNumber.length !== 10) {
                            validationErrors.push(`Invalid phone number in row: ${JSON.stringify(row)}`);
                            continue;
                        }

                        if (category && !['herbivores', 'carnivores', 'omnivores', 'amphibian', 'reptiles'].includes(category)) {
                            validationErrors.push(`Invalid category in row: ${JSON.stringify(row)}`);
                            continue;
                        }

                        if (dob && new Date(dob) > new Date()) {
                            validationErrors.push(`Invalid date of birth in row: ${JSON.stringify(row)}`);
                            continue;
                        }
                    ...
                    if (validAnimalsData.length > 0) {
                        try {
                            ...
                            return response.status(200).json({
                                status: "success",
                                message: "Animals created successfully, password would be shared to mail, but you can access_password first fourletter following with 1234567",
                                data: createdAnimals,
                            });
                        } catch (error) {
                ...
                .on('error', (err) => {
                    return response.status(500).json({ status: "error", message: "Error parsing the CSV file", error: err.message });
                });
        } catch (error) {
            console.log(error);
            return response.status(500).json({
